  ...
  <section id="home" className="w-full h-screen text-center">
    <div className="max-w-[1240px] w-full h-full mx-auto p-2 flex justify-center items-center">
      <div>
        ...
        <a target="_blank" rel="noreferrer" className='px-8 py-4 text-sm shadow-lg inline-block px-4 shadow-gray-400 rounded-lg uppercase bg-gradient-to-r from-[#5651e5] to-[#709dff] text-white' href='/assets/cv/Resume.pdf'>GET MY RESUME</a>
        <ul className="flex items-center justify-between max-w-[330px] m-auto py-4">
          {EXTERNAL_LINKS.map((exLink) => (
            <li key={exLink.linkUrl}>
              <a href={exLink.linkUrl} target="_blank" rel="noreferrer" className="rounded-full inline-block shadow-lg shadow-gray-400 p-6 cursor-pointer hover:scale-105 ease-in duration-500" >{exLink.icon}</a>
            </li>
          ))}
        </ul>
      </div>
